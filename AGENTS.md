...
| Pi host | `rpi-home` — `203.0.113.10`, user `kapekost`, SSH key `~/.ssh/<deploy-key>` |
...
| App URL (LAN) | `http://203.0.113.10:8080` |
...
docker save kapekost/workout-tracker:latest | gzip | \
  ssh deploy-user@example-pi-host 'gunzip | docker load'
```

### Run / update (Pi)
```bash
ssh deploy-user@example-pi-host
cd ~/workout-tracker
...
### Verify (every deploy)
```bash
curl -s -o /dev/null -w "%{http_code}\n" http://203.0.113.10:8080/      # expect 200
...
ssh deploy-user@example-pi-host 'docker compose -f ~/workout-tracker/docker-compose.yml ps; \
...
- ~~2026-07-10, after 03:30 — first real cron run~~ **verified in full
  ...
  (SSH note: use plain `ssh deploy-user@example-pi-host` — `BatchMode=yes` blocks
...
**Blocked on user**
- **HTTPS domain** (`https://example-pi-host.tailnet.ts.net` via Tailscale
