  ...
  return (
    <div role="dialog" aria-modal="true" aria-label={`${ex.name} form cues`}
      ...
      <div onClick={e => e.stopPropagation()} className="max-w-md mx-auto"
        style={{
          background: '#0a0a12', borderTopLeftRadius: 20, borderTopRightRadius: 20,
          width: '100%', maxHeight: '85vh', overflowY: 'auto',
          padding: '20px 16px calc(24px + env(safe-area-inset-bottom))'
        }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 4 }}>
          <h2 style={{ fontSize: '1.3rem', fontWeight: 700 }}>{ex.name}</h2>
          <button className="btn-icon tap-target" aria-label="close" onClick={onClose}>×</button>
        </div>
        {ex.alt && <p style={{ color: '#6b7280', fontSize: '0.8rem', marginBottom: 16 }}>{ex.alt}</p>}
