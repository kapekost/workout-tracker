...
|---|------|----------|---------|---------|--------------|--------|
| R1 | Workout | 320–375 | TimerBar controls row (−30 / REST clock / +30 / ⏸ / Skip) | Cluster overflows the viewport: **Skip fully off-screen at 320** (right edge 371/320), clipped at 360 (379/360) and 375 (379/375). Controls unusable while resting on small phones. | `@media (max-width: 400px)`: tighten bar padding/gaps, shrink REST clock (font + min-width), trim ⏸/Skip padding. Keep all five controls visible and ≥44px tall. | Open |
...
| R3 | Workout | all (latent) | PR toast (`.toast`) | `white-space: nowrap` + long exercise name (e.g. "🏆 PR! 57.5kg on Standing Calf Raise") exceeds narrow viewports; current seeds happen to fit at 320. | `.toast { max-width: calc(100vw - 32px); white-space: normal; text-align: center }`. | Open |
...
| R10 | TimerBar | all | ⏸ button | 32px wide (44 tall) — narrowest control in the bar. | `min-width` bump within the R1 rework where space allows. | Open |
