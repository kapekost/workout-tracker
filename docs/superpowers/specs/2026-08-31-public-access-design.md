...
and the stakes here are a home network that also runs a co-located service, not just this app. Needs an
...
design problem, not a quick config change: **the same Pi's home network also runs another
service.** Whatever ships must not create any new path toward that service or the rest of the
...
alone is transport, not access control (§2); an explicit, checkable verification list proving the
co-located service and the rest of the LAN stay unreachable through the new public hostname (§5); sequencing
against real login (§6).

**Out:** migrating off the Pi (explicitly decided against). Any change to the co-located service's own
setup — "scoped tightly to this app's own service/port, not the LAN" cuts both ways: this issue
doesn't touch the co-located service, doesn't add it to the tunnel, and doesn't audit its configuration.
...
| Co-located-service coexistence? | Hard requirement: scoped tightly to this app's own service/port, not the LAN. Needs a real home-network security review. |
...
our profiles are they." Given the co-located-service constraint, defense in depth here is deliberate,
...
- **The co-located service is never added to this tunnel's ingress rules**, as part of this issue or ever
...
- **This is this spec's proposal, not a recorded owner decision** — flagged explicitly so it's easy
  ...
  Access). Given the co-located-service stakes, this document's recommendation is to keep both layers.
...
- [ ] From an external network, attempting to reach the co-located service's own hostname/port through the
...
- [ ] `AGENTS.local.md`'s "Co-located services" section names it and records this
...
Whether or how the co-located service itself might ever be made reachable from outside the home network is
