...
- [ ] **Step 3: Transfer** — `docker save kapekost/workout-tracker:latest | gzip | ssh -i ~/.ssh/<deploy-key> deploy-user@example-pi-host 'gunzip | docker load'`
- [ ] **Step 4: Restart on Pi** — `ssh -i ~/.ssh/<deploy-key> deploy-user@example-pi-host 'cd ~/workout-tracker && docker compose up -d'`
