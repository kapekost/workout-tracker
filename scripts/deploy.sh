...
if [[ ! -f "$LOCAL_DOC" ]]; then
  echo "error: AGENTS.local.md is required for deployment-specific settings" >&2
  exit 1
fi
...
: "${DEPLOY_HOST:?AGENTS.local.md must define DEPLOY_HOST}"
: "${DEPLOY_APP_DIR:?AGENTS.local.md must define DEPLOY_APP_DIR}"
...
# tree so /api/health and the UI footer cannot claim a source state that was
# never committed.
