...
def test_set_password_with_a_valid_invite_logs_the_user_in(fast, anon_client):
    pid = _profile(fast)
    with fast.db() as conn:
        raw = fast.mint_token(conn, pid, "invite")
        conn.commit()
    r = anon_client.post("/api/auth/set-password", json={"token": raw, "password": "correct horse battery"})
...
def test_a_token_is_single_use(fast, anon_client):
    pid = _profile(fast)
    with fast.db() as conn:
        raw = fast.mint_token(conn, pid, "invite")
        conn.commit()
...
def test_expired_unknown_and_used_tokens_share_one_generic_message(fast, anon_client):
    pid = _profile(fast)
    with fast.db() as conn:
        expired = fast.mint_token(conn, pid, "invite")
        conn.execute("UPDATE auth_tokens SET expires_at = datetime('now','-1 day')")
        conn.commit()
    a = anon_client.post("/api/auth/set-password", json={"token": expired, "password": "correct horse battery"})
    b = anon_client.post("/api/auth/set-password", json={"token": "never-existed", "password": "correct horse battery"})
    assert a.status_code == b.status_code == 400
    assert a.json()["detail"] == b.json()["detail"]


def test_set_password_enforces_the_password_rules(fast, anon_client):
    pid = _profile(fast)
    with fast.db() as conn:
        raw = fast.mint_token(conn, pid, "invite")
        conn.commit()
    r = anon_client.post("/api/auth/set-password", json={"token": raw, "password": "short"})
...
def test_set_password_revokes_every_existing_session(fast, anon_client):
...
def test_forgot_password_is_identical_for_known_and_unknown_addresses(fast, anon_client, sent):
    _profile(fast, email="known@example.com")
    known = anon_client.post("/api/auth/forgot-password", json={"email": "known@example.com"})
    unknown = anon_client.post("/api/auth/forgot-password", json={"email": "nobody@example.com"})
    assert known.status_code == unknown.status_code == 200
    assert known.json() == unknown.json()


def test_forgot_password_emails_only_a_real_address(fast, anon_client, sent):
    _profile(fast, email="known@example.com")
    anon_client.post("/api/auth/forgot-password", json={"email": "known@example.com"})
    anon_client.post("/api/auth/forgot-password", json={"email": "nobody@example.com"})
    assert [m["to"] for m in sent] == ["known@example.com"]
    with fast.db() as conn:
        assert conn.execute("SELECT COUNT(*) FROM auth_tokens").fetchone()[0] == 1


def test_forgot_password_on_a_never_invited_account_still_works(fast, anon_client, sent):
    ...
    r = anon_client.post("/api/auth/forgot-password", json={"email": "fresh@example.com"})
    assert r.status_code == 200
    assert len(sent) == 1
...
def test_the_emailed_link_uses_app_base_url_and_carries_the_raw_token(fast, anon_client, sent, monkeypatch):
...
def _admin_session(fast, anon_client):
    with fast.db() as conn:
...
def test_creating_a_profile_requires_an_admin_session(fast, anon_client, sent):
    anon = anon_client.post("/api/profiles", json={"username": "newbie", "email": "n@example.com"})
...
def test_admin_creating_a_profile_mints_an_invite_and_emails_it(fast, anon_client, sent):
    c = _admin_session(fast, anon_client)
...
def test_a_failed_send_still_creates_the_profile_and_says_so(fast, anon_client, monkeypatch):
...
def test_a_duplicate_username_or_email_is_rejected(fast, anon_client, sent):
    c = _admin_session(fast, anon_client)
...
def test_login_429s_after_ten_attempts_in_the_window(limited, anon_client):
...
def test_the_limiter_blocks_the_correct_password_too(limited, anon_client):
    _profile(limited, username="tester", email="t@example.com",
             password_hash=limited.hash_password("correct horse battery"))
    for _ in range(10):
    ...
    r = anon_client.post("/api/auth/login", json={"username": "tester", "password": "correct horse battery"})
    assert r.status_code == 429
    assert "set-cookie" not in r.headers


def test_a_rate_limited_login_does_no_hashing(limited, anon_client, monkeypatch):
    _profile(limited, username="tester", email="t@example.com",
             password_hash=limited.hash_password("correct horse battery"))
    bad = {"username": "tester", "password": "wrong horse battery"}
    for _ in range(10):
        anon_client.post("/api/auth/login", json=bad)
    calls = []
...
def test_forgot_password_is_rate_limited_too(limited, anon_client, sent):
    _profile(limited, email="known@example.com")
    body = {"email": "known@example.com"}
    codes = [anon_client.post("/api/auth/forgot-password", json=body).status_code for _ in range(10)]
    assert codes == [200] * 10
    assert anon_client.post("/api/auth/forgot-password", json=body).status_code == 429


def test_the_window_expires(limited, anon_client, monkeypatch):
    _profile(limited, username="tester", email="t@example.com",
             password_hash=limited.hash_password("correct horse battery"))
    bad = {"username": "tester", "password": "wrong horse battery"}
    for _ in range(10):
...
def test_the_ip_counter_also_bites_across_usernames(limited, anon_client):
    _profile(limited, username="tester", email="t@example.com",
             password_hash=limited.hash_password("correct horse battery"))
    ...
    r = anon_client.post("/api/auth/login", json={"username": "other", "password": "another good password"})
    assert r.status_code == 429


def test_a_different_source_ip_is_counted_separately(limited, anon_client):
    _profile(limited, username="tester", email="t@example.com",
             password_hash=limited.hash_password("correct horse battery"))
    for _ in range(11):
        anon_client.post("/api/auth/login", json={"username": "tester", "password": "wrong horse battery"})
...
def test_bootstrap_goes_through_the_normal_invite_path_not_a_backdoor(fast, sent, anon_client):
